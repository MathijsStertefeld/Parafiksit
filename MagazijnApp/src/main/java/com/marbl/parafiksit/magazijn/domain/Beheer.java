package com.marbl.parafiksit.magazijn.domain;

import com.marbl.parafiksit.magazijn.APInterface;
import com.marbl.parafiksit.magazijn.IFactuur;
import com.marbl.parafiksit.magazijn.IFactuurRegel;
import com.marbl.parafiksit.magazijn.IKlant;
import com.marbl.parafiksit.magazijn.IMagazijn;
import com.marbl.parafiksit.magazijn.IOnderdeel;
...
public class Beheer implements IMagazijn, APInterface {
    ...
    //Constructor
    /**
     * Maakt een nieuwe databaseConnectie-klasse aan. Deze geeft toegang tot de
     * methoden die nodig zijn om de database te benaderen.
     */
    public Beheer() {
        try {
     ...
     *
     * @return Een lijst van IOnderdeel objecten (Interface). Als er iets fout
     ...
     *
     * @return Een lijst van IKlant objecten (Interface). Als er iets fout
     * gegaan is wordt een lege ArrayList gereturneerd. (Let dan wel op, want
     * het kan ook zijn dat de query geen row teruggeeft!)
     */
     ...
     *
     * @return Een lijst van IFactuur objecten (Interface). Als er iets fout
     * gegaan is wordt een lege ArrayList gereturneerd. (Let dan wel op, want
     * het kan ook zijn dat de query geen row teruggeeft!)
     */
     ...
     * het aantal of de prijs minder dan 0 is, de omschrijving null of gelijk is
     * aan een lege string wordt de actie onderbroken: Returns -1.
     *
     * @param code De code van het Onderdeel (zoals deze in de database wordt
     * opgeslagen).
     * @param omschrijving De omschrijving van het Onderdeel (zoals deze in de
     * database wordt opgeslagen).
     * @param aantal Het x aantal keer dat het product in het magazijn wordt
     * toegevoegd.
     * @param prijs De prijs van het Onderdeel (zoals deze in de database wordt
     * opgeslagen).
    ...
    /**
     * Zoekt een geschikt klantnummer en voegt de nieuwe klant toe aan de
     * database. Als de parameters gelijk zijn aan null of gelijk zijn aan een
     * lege string wordt de actie afgebroken: Returns -1.
     *
     * @param naam De naam van de Klant (Zoals deze in de database wordt
     * opgeslagen).
     * @param adres Het aders van de Klant (Zoals deze in de database wordt
     * opgeslagen).
     ...
     * database als de variabelen kloppen. Ook wordt het aantal van de
     * betreffende onderdelen in de database verlaagt met het aantal te
     * bestellen van dat onderdeel.
     *
     * @param klantId Het ID van de klant die de factuur aanvraagt.
     * @param onderdelen Lijst met onderdelen die besteld worden: Array van
     * int[], waarvan int[0] de onderdeelcode is en int[1] het aantal te
     * bestellen.
...
//</editor-fold>
     ...
     * zoja wordt deze verwijdert.
     *
     * @param onderdeelCode De Code die hoort bij het onderdeel wat verwijdert
     * moet worden.
...
//</editor-fold>
     ...
     * adres!!) Controleert of het ingevoerde IKlant-object correct is. Als dit
     * zo is worden de veranderen aangebracht in de database. Zowel de naam als
     * het adres moet ingevoerd worden, als deze niet veranderd hoeven worden
     * moeten de oude waardes ingevoerd worden.
     *
     ...
     * omschrijving het aantal en de prijs!) Controleert of het ingevoerde
     * IOnderdeel-obejct correct is. Als dit zo is worden de veranderingen
     * aangebracht in de database. De omschrijving, het aantal en de prijs
     * moeten ingevoerd worden, als er niets veranderd hoeft te worden, voer dan
     * de oude waardes in.
     *
     ...
     *
     * @param onderdeelCode De code van het onderdeel waarvan de informatie moet
     * worden opgehaald.
     * @return Een Object[] met de volgende waardes: [0] String: De omschrijving
     ...
     * variabelen kloppen, als de variabelen niet kloppen wordt -1 gereturneerd.
     * Ook wordt het aantal van de betreffende onderdelen in de database
     * verlaagt met het aantal te bestellen van dat onderdeel, als er niet
     * genoeg onderdelen over zijn wordt ook -1 gereturneerd.
     *
     * @param klantId Het ID van de klant die de factuur aanvraagt.
     * @param onderdelen Lijst met onderdelen die besteld worden: De ArrayList
     * heeft int[] objecten, en elke int[] staat voor een onderdeel: int[0]: De
     * code van het onderdeel. int[1]: Het aantal van dat onderdeel dat besteld
     * moet worden.
     * @return Als de factuur correct wordt toegevoegd wordt het factuurnummer
     * gereturneerd, anders wordt -1 gereturneerd.
     */

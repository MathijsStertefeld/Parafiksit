...
abstract class ClientRequestProcess {
    ...
    private void requestCreditHistory() {
        CreditRequest cRequest = createCreditRequest(clientRequest);
        creditGateway.getCreditHistory(cRequest, creditReplyListener);
    }
    ...
    public void onCreditReply(CreditReply reply) {
        System.out.println("Loanbroker received reply from creditbureau");
        creditReply = reply;
    ...
    public void onBankQuoteReply(BankQuoteReply reply) {
        System.out.println("Loanbroker receiver reply from Bank");
